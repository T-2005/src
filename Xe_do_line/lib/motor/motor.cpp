...
void motor::motion(int speedL, int speedR)
{
    ...
    {
        motorControl(_pwmChannelR, _in1R, _in2R, speedR, false);
        motorControl(_pwmChannelL, _in1L, _in2L, speedL, true);
    }
    else if (speedL < 0)
    {
        motorControl(_pwmChannelR, _in1R, _in2R, speedR, true);
        motorControl(_pwmChannelR, _in1L, _in2L, speedL, false);
    }
...
float motor ::PID_control(float setValue, float readValue, uint32_t lastTimePID, float Kp, float Ki, float Kd)
{
    ...
    Serial.print("Gia tri output cua PID_control: ");
    Serial.println(output);

    Serial.print("Gia tri output cua deltaTime: ");
    Serial.println(deltaTime);
    Serial.print("Gia tri output cua lastError: ");
    Serial.println(lastError);

    deltaTime = (float)(millis() - lastTimePID) / 1000.0;

    error = setValue - readValue;
    Serial.print("Gia tri output cua error: ");
